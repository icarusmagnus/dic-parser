...
<th>Assureur</th><th>Contrat</th><th>Type</th><th>SRI</th><th>Détention</th><th>Frais</th><th>Complétude</th><th>DIC</th>
</tr></thead><tbody></tbody></table></div>
...
function crow(c){
  ...
  return `<tr><td>${c.insurer||''}</td><td>${c.name||''}</td><td>${c.type||''}</td>
...
document.querySelector('#qc').addEventListener('input',e=>{const q=e.target.value.toLowerCase();
  crows=(CONTRACTS.contracts||[]).filter(c=>[c.insurer,c.name,c.type].join(' ').toLowerCase().includes(q));crender();});
crender();
